...
class BiasMeasurementCompiler:
    ...
    def compile(self):
        ...
        #         # ... [Your existing code]
        #
        #         # Extract data for the axis named "ability"
            axis_name = "ability"
            data_for_csv = []

            if axis_name in binned_perplexities:
                for template in binned_perplexities[axis_name]:
                    for descriptor in binned_perplexities[axis_name][template]:
                        for ppl in binned_perplexities[axis_name][template][descriptor]:
                            data_for_csv.append([template, descriptor, ppl])

            # Convert the list of lists to a pandas DataFrame
            df = pd.DataFrame(data_for_csv, columns=["Template", "Descriptor", "Perplexity"])

            # Save the DataFrame to a CSV file
            csv_save_path = os.path.join(save_folder, f"perplexities_for_{axis_name}.csv")
            df.to_csv(csv_save_path, index=False)
            print(f"Saved binned perplexities for axis '{axis_name}' to {csv_save_path}.")
